...
async function generateReferralCode(): Promise<string> {
  ...
  return `AP-${randomUUID().replace(/-/g, '').slice(0, 8).toUpperCase()}`;
}
...
export async function createUserAction(
  ...
  if (!name || name.length < 2)  return { ok: false, message: 'Nome deve ter ao menos 2 caracteres.' };
  if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return { ok: false, message: 'E-mail inválido.' };
  if (password.length < 8)       return { ok: false, message: 'Senha deve ter ao menos 8 caracteres.' };
...
export async function updateUserPlanAction(
  ...
  try {
    ...
    // Recalcula comissões NÃO PAGAS (ambas direções: como referido e como sponsor)
    await adminRepository.recalculateUserCommissions(userId, monthlyValueCents ?? 0);
...
const VALID_CAREERS = [
  'vendedor_elite', 'supervisor', 'gestor', 'gerente_senior', 'diretor_geral',
];
...
export async function updateCareerAction(
  ...
  try {
    ...
    revalidatePath('/portal/cashback');
    revalidatePath('/portal/saldo');
    return { ok: true, message: finalCareer ? 'Carreira atualizada.' : 'Carreira removida (automática).' };
  } catch (err) {
...
export async function updateAdhesionPaidAction(
  ...
  try {
    ...
    revalidatePath('/portal/carreira');
    revalidatePath('/portal/saldo');
    return { ok: true, message: paid ? 'Adesão marcada como paga.' : 'Adesão marcada como não paga.' };
  } catch (err) {
...
export async function markCashbackMonthAction(
  ...
  const paidBy = (formData.get('paidBy') as string | null)?.trim() ?? 'admin';
  ...
  try {
    await adminRepository.markCashbackMonthPaid(userId, monthNum, amountCents, paidBy);
    revalidatePath('/admin/cashback');
    revalidatePath('/admin/comissoes');
    revalidatePath('/portal', 'layout');
