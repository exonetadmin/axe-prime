...
const TYPE_META: Record<string, { label: string; color: string }> = {
  direct:  { label: 'Direto (10%)',  color: 'var(--accent)' },
  network: { label: 'Rede (2%)',     color: '#a78bfa'        },
};
      ...
      <header className="adm-page-header">
        <div>
          ...
          <p className="adm-page-sub">
            ...
            <span style={{ marginLeft: '0.75rem', color: 'var(--text-soft)' }}>· {total} registros</span>
          </p>
        </div>
          ...
          return (
            <div key={t.type} className="adm-stat-card">
              ...
              <div className="adm-stat-body">
                ...
                <p className="adm-stat-label">{meta.label} · {t.count} reg.</p>
              </div>
            </div>
          );
        })}
          ...
          <tbody>
            {rows.length === 0 ? (
              ...
              rows.map((c) => {
                ...
                const statusMeta = STATUS_META[c.status] ?? { label: c.status, color: 'var(--text-soft)' };
                return (
                  <tr key={c.id}>
                    <td data-label="Patrocinador" className="adm-table-name">{c.sponsor_name}</td>
                    <td data-label="Indicado" className="adm-table-muted">{c.referred_name}</td>
                    <td data-label="Tipo">
                      <span className="adm-badge" style={{ color: typeMeta.color, borderColor: `${typeMeta.color}28` }}>
                        {typeMeta.label}
                      </span>
                    </td>
                    <td data-label="Nível" className="adm-table-muted" style={{ fontVariantNumeric: 'tabular-nums' }}>
                      N{c.level}
                    </td>
                    <td data-label="Valor" className="adm-table-value">{fmt(c.amount_cents)}</td>
                    <td data-label="Período" className="adm-table-muted" style={{ fontVariantNumeric: 'tabular-nums' }}>{c.period}</td>
                    <td data-label="Status">
                      <span className="adm-badge" style={{ color: statusMeta.color, borderColor: `${statusMeta.color}28` }}>
                        {statusMeta.label}
                      </span>
                    </td>
